  ...
  <div class="main-wrapper">
    ...
    <div class="form-wrapper d-flex align-items-center justify-content-center">
      <form class="col-lg-2">
        ...
        <button type="submit" class="btn btn-primary mx-3" v-on:click.prevent="submitLogin()">
          Login
        </button>
        <router-link :to="{ name: 'Home' }">
          <button class="btn btn-danger">back</button>
        </router-link>
        <router-link class="px-3 link-options" :to="{ name: 'signup' }">SignUp instead</router-link>
      </form>
    </div>
  </div>
</template>
